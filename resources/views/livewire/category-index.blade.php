        ...
        <div class="mx-6 mt-20">
            <div class="flex justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <a href="#">
                        <div class="font-medium text-lg">Kategori</div>
                    </a>
                </div>
            </div>
        </div>
                ...
                <table class="w-full text-sm text-left text-gray-600">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-white border-b hover:bg-gray-100 hover:text-black font-medium">
                            <td class="py-4 px-6">{{ ($categories ->currentpage()-1) * $categories ->perpage() + $loop->index + 1 }}</td>
                            <td class="py-4 px-6">{{ $category->name }}</td>
                            <td class="py-4 px-6">{{ $category->created_at->format('m/d/Y') }}</td>
                            <td class="py-4 px-6">{{ $category->updated_at->format('m/d/Y') }}</td>
                            <td class="py-4 px-6">
                                <div class="flex items-center gap-4">
                                    <button title="Edit" wire:click="showCategoryEditModal( {{ $category->id }} )">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                    </button>
                                    <button title="Hapus" wire:click="deleteCategory({{ $category->id }})">
            ...
            <div class="bg-black bg-opacity-50 fixed inset-0 flex justify-center items-center">
                <div class="bg-white p-4 rounded-lg shadow-md">
                    <div class="flex justify-between items-center">
                        ...
                        @else
                            <h1 class="font-medium text-xl">Tambah Kategori</h1>
                        @endif
